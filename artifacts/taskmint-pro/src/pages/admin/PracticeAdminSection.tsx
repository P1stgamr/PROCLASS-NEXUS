...
export default function PracticeAdminSection() {
  ...
  const parseRow = (row: any) => {
    ...
    return { subject: String(pick("subject", "Subject") || "math").toLowerCase(), topic: String(pick("topic", "Topic") || "General"), difficulty: String(pick("difficulty", "Difficulty") || "Medium"), question: String(pick("question", "text", "Question") || ""), options: finalOptions.map(String), correct: Math.max(0, correct - 1), explanation: String(pick("explanation", "Explanation") || ""), points: 1, updatedAt: Date.now() };
  };
      ...
      {tab === "questions" ? <div className="grid xl:grid-cols-[380px_1fr] gap-5">
        <div className="space-y-4">
          <form onSubmit={saveQuestion} className="glass-card rounded-2xl p-5 space-y-3">
            ...
            <select value={question.subject} onChange={(e) => setQuestion({ ...question, subject: e.target.value })} className={`w-full h-10 rounded-xl px-3 text-sm ${FIELD}`}>{SUBJECTS.map((s) => <option key={s} value={s}>{s}</option>)}</select>
        ...
        <form onSubmit={saveTest} className="glass-card rounded-2xl p-5 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2"><select value={test.subject} onChange={(e) => setTest({ ...test, subject: e.target.value })} className={`h-10 rounded-xl px-3 text-sm ${FIELD}`}>{SUBJECTS.map((s) => <option key={s}>{s}</option>)}</select><Input value={test.topic} onChange={(e) => setTest({ ...test, topic: e.target.value })} placeholder="Topic (optional)" className={FIELD} /></div>
